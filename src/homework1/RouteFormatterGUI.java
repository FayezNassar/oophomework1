...
public class RouteFormatterGUI extends JPanel {
    ...
    public void addSegment(GeoSegment segment) {
        ...
        if (route == null) {
            ...
            if (!segment.getP1().equals(route.endingGeoSegment.getP2())) {
                return;
            }
            route = route.addSegment(segment);
        }
        DrivingRouteFormatter drivingRouteFormatter = new DrivingRouteFormatter();
        WalkingRouteFormatter walkingRouteFormatter = new WalkingRouteFormatter();

        String drivingRoute = drivingRouteFormatter.computeDirections(route, 0.0);
        String walkingRoute = walkingRouteFormatter.computeDirections(route, 0.0);

        model.addElement(segment);
        txtDrivingDirections.setText(drivingRoute);
        txtWalkingDirections.setText(walkingRoute);
